...
// ── Analysis Tab (extracted to use hooks safely) ──────────────────────────────
function AnalysisTab({
  windParams, setWindParams, windFetched,
  weibullData, totalAEP_live, liveCapFactor,
  avgWindSpeed, totalCableLength, totalCableCost, monthlyData,
}) {
  const [goal, setGoal] = useState(window.__analysisGoal__ || '');

  return (
    <div className="space-y-4">

      {/* Optimisation Goal Selector */}
      <div className="bg-slate-800/60 rounded-xl p-3">
        <p className="text-[10px] text-slate-400 uppercase tracking-wider font-medium mb-2">🎯 Optimisation Goal</p>
        <div className="space-y-1.5">
          {[
            { id: 'energy',     label: 'Maximise AEP',         desc: 'Push for highest annual energy output' },
            { id: 'cost',       label: 'Minimise Cable Cost',   desc: 'Reduce infrastructure spend' },
            { id: 'compliance', label: 'Maximise Compliance',   desc: 'Maximise constraint clearances' },
          ].map(g => (
            <button key={g.id}
              onClick={() => {
                window.__analysisGoal__ = g.id;
                setGoal(g.id);
                window.__trainingEvent__ = { type: 'analysis_goal_selected', payload: { goal: g.id }, ts: Date.now() };
              }}
              className={cn('w-full text-left px-2.5 py-2 rounded-lg border transition-all text-xs',
                goal === g.id
                  ? 'bg-emerald-500/15 border-emerald-500/40 text-emerald-300'
                  : 'bg-slate-800 border-slate-700 text-slate-400 hover:text-white hover:border-slate-500'
              )}>
              <p className="font-medium">{g.label}</p>
              <p className="text-[9px] text-slate-500 mt-0.5">{g.desc}</p>
            </button>
          ))}
        </div>
      </div>
          ...
          {[
            ...
            <div key={key}>
              ...
              <input type="range" min={min} max={max} step={step} value={windParams[key]}
                onChange={e => {
                  setWindParams(p => ({ ...p, [key]: parseFloat(e.target.value) }));
                  window.__trainingEvent__ = { type: 'weibull_changed', payload: { key, value: e.target.value }, ts: Date.now() };
                }}
  ...
  return (
    <div className={cn('shrink-0 flex flex-col bg-slate-900 border-l border-slate-800 overflow-hidden transition-all duration-200', rightPanelOpen ? 'w-80' : 'w-0 border-l-0')}>
      ...
      <div className="flex-1 overflow-y-auto p-3">
        ...
        {rightTab === 'turbines' && (
          <TurbineDataTable
            turbines={turbines} turbineTypes={turbineTypes}
            onDeleteTurbine={(id) => turbineLayer && deleteFeature(turbineLayer.id, id)}
            onUpdateTurbine={updateTurbineProps} onFlyTo={flyToFeature}
            globalRadii={globalRadii} onRadiiChange={onRadiiChange}
            showRadii={showRadii} onToggleRadii={onToggleRadii}
          />
        )}

        {rightTab === 'cables' && (
          <CableDataTable
            cables={cables} cableTypes={cableTypes}
            selectedCableTypeId={selectedCableTypeId} onSelectCableType={setSelectedCableTypeId}
            ...
            onUpdateCable={(id, props) => {
              ...
              updateLayer(cableLayer.id, { features: cableLayer.features.map(f => f.id === id ? { ...f, properties: props } : f) });
            }}
            onOptimiseCables={() => {
              if (!cableLayer) return;
            ...
            turbines={turbines} substations={substations} onFlyTo={flyToFeature}
          />
        )}

        {rightTab === 'analysis' && (
          <AnalysisTab
            windParams={windParams} setWindParams={setWindParams} windFetched={windFetched}
            weibullData={weibullData} totalAEP_live={totalAEP_live} liveCapFactor={liveCapFactor}
