...
import { FolderOpen, Plus, Trash2, Save, X, FileText, Clock, ChevronRight } from 'lucide-react';
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {isCurrent && (
                          <span className="text-[9px] bg-emerald-500/20 text-emerald-400 px-1.5 py-0.5 rounded font-semibold shrink-0">OPEN</span>
                        )}
                      </div>
...
export default function ProjectFileButtons({ currentProjectId, currentProjectName, currentData, onNewProject, onSwitchProject, onSaved }) {
  const [modal, setModal] = useState(null); // null | 'open' | 'save'
  ...
  return (
    <>
      <div className="flex items-center gap-1">
        <button
          onClick={handleNew}
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs bg-slate-800 border border-slate-700 text-slate-300 hover:text-white hover:border-slate-500 transition-all"
          title="New Project"
        >
