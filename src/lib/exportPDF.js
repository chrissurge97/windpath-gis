...
function setDrawColor(doc, hex) { const [r, g, b] = hexToRgb(hex); doc.setDrawColor(r, g, b); }
...
// ── Leaflet map capture via html2canvas ──────────────────────────────────────
...
async function addMapPage(doc, map, layers, projectName) {
  ...
  if (visibleTypes.has('cable')) {
    const cl = (layers || []).find(l => l.type === 'cable' && l.visible);
    legLine('Cable', cl?.color || '#f97316', 'line');
  }
  ...
  if (polyLayers.length > 0) {
    ...
    ly += 8;
    for (const layer of polyLayers) legLine(layer.name, layer.color || '#06b6d4', 'rect');
  }
...
export async function exportProjectPDF({
  ...
  totalCapacity_mw, totalAEP, avgCapFactor, avgWindSpeed,
  totalCableLength, totalCableCost, windParams, monthlyData, layers, mapRef,
}) {
  ...
  const tileW = CW / 3, tileH = 20;
  kpis.forEach((k, i) => {
    const tx = ML + (i % 3) * tileW, ty = y + Math.floor(i / 3) * (tileH + 2);
    setFill(doc, '#1e293b');
  ...
  [
    ...
    setTextColor(doc, '#94a3b8'); doc.setFontSize(8); doc.setFont('helvetica', 'normal'); doc.text(label, ML + 3, y + 4.2);
    setTextColor(doc, '#e2e8f0'); doc.setFont('helvetica', 'bold'); doc.text(value, ML + 65, y + 4.2);
    setTextColor(doc, '#475569'); doc.setFont('helvetica', 'normal'); doc.setFontSize(7); doc.text(note, ML + 100, y + 4.2);
    y += 6;
  });
  y += 4;
  ...
  const cols = [
    { label: 'Name', x: ML + 2 }, { label: 'Type', x: ML + 24 }, { label: 'Rated MW', x: ML + 71 },
    { label: 'Hub Wind', x: ML + 93 }, { label: 'Est. AEP (MWh)', x: ML + 117 }, { label: 'Elev (m)', x: ML + 151 },
  ];
  setFill(doc, '#0f172a'); doc.rect(ML, y, CW, 6, 'F');
  cols.forEach(c => { setTextColor(doc, '#64748b'); doc.setFontSize(7); doc.setFont('helvetica', 'bold'); doc.text(c.label, c.x, y + 4.2); });
  y += 6;
  const maxRows = Math.min(turbines.length, 20);
  turbines.slice(0, maxRows).forEach((t, i) => {
    const p = t.properties, tt = turbineTypes.find(ty => ty.id === p.turbine_type_id);
    ...
    [
      ...
      { c: cols[2], v: String(p.rated_power_mw || tt?.rated_power_mw || '—'), color: '#06b6d4' },
    ...
    ].forEach(({ c, v, color }) => { setTextColor(doc, color); doc.setFontSize(7.5); doc.setFont('helvetica', 'normal'); doc.text(String(v), c.x, y + 4.2); });
    y += 6;
  });
  if (turbines.length > maxRows) { setTextColor(doc, '#64748b'); doc.setFontSize(7); doc.text(`… and ${turbines.length - maxRows} more turbines`, ML + 3, y + 4); y += 7; }
  y += 5;
  ...
  if (cables.length > 0) {
    ...
    const cableCols = [
      { label: 'Name', x: ML + 2 }, { label: 'Type', x: ML + 32 }, { label: 'Length (km)', x: ML + 74 },
      { label: 'Voltage (kV)', x: ML + 104 }, { label: 'Cost (£)', x: ML + 134 },
    ];
    setFill(doc, '#0f172a'); doc.rect(ML, y, CW, 6, 'F');
    cableCols.forEach(c => { setTextColor(doc, '#64748b'); doc.setFontSize(7); doc.setFont('helvetica', 'bold'); doc.text(c.label, c.x, y + 4.2); });
    y += 6;
    const maxCables = Math.min(cables.length, 15);
    cables.slice(0, maxCables).forEach((c, i) => {
      const p = c.properties, ct = cableTypes.find(t => t.id === p.cable_type_id) || cableTypes[0];
      ...
      ].forEach(({ c: col, v, color }) => { setTextColor(doc, color); doc.setFontSize(7.5); doc.setFont('helvetica', 'normal'); doc.text(String(v), col.x, y + 4.2); });
      y += 6;
    });
    if (cables.length > maxCables) { setTextColor(doc, '#64748b'); doc.setFontSize(7); doc.text(`… and ${cables.length - maxCables} more cables`, ML + 3, y + 4); y += 7; }
    y += 2;
    setFill(doc, '#1e293b'); doc.rect(ML, y, CW, 7, 'F');
    setTextColor(doc, '#94a3b8'); doc.setFontSize(7.5); doc.setFont('helvetica', 'bold');
    doc.text(`Total: ${cables.length} cables`, ML + 3, y + 4.8);
    setTextColor(doc, '#f97316'); doc.text(`${(totalCableLength / 1000).toFixed(2)} km`, ML + 74, y + 4.8);
    setTextColor(doc, '#f59e0b'); doc.text(`£${(totalCableCost / 1000).toFixed(0)}k`, ML + 134, y + 4.8);
    y += 10;
  }

  // Footer
  const footerY = PH - 12;
  setFill(doc, '#0f172a'); doc.rect(0, footerY - 4, PW, 16, 'F');
  setFill(doc, '#10b981'); doc.rect(0, footerY - 4, PW, 1, 'F');
  setTextColor(doc, '#475569'); doc.setFontSize(7); doc.setFont('helvetica', 'normal');
